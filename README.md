...
<details>
    ...
    다크모드 고려해서 이벤트 탭 UI 바꾸기 현재 다크모드에서 이벤트 헤더 안보임, 이벤트 호버 시 색 이상함, 이벤트 등록 호버도 이상
    프로필 사진 넣었을 때 탭 오른쪽에 약간 치우치는거 고치기
    다크모드일때 subhub 색깔 적당히 괜찮게 바꾸기
...
> 문제점
* 결제 정보 긁어오기 가능 여부 (은행 api 받아오기 사실상 불가능)
* 사람마다 다른 결제일들 ex)연간 구독
